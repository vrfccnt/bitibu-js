//EXAMPLE Market Making Bot using the bitibu API
		...
		if(!res.error){
			...
			bitibu.orderBook(theMarket, function(res){
				if(!res.error){
					...
					var oursSell = false;
					var oursBuy = false;
					bitibu.orders("onionbtb", function(res){
						if(!res.error){
							...
							if(!oursBuy || !oursSell){
								...
								bitibu.createOrder("onionbtb", "buy", volume, buyPrice, function(res){
									if(!res.error){
											//sell
											bitibu.createOrder("onionbtb", "sell", volume, sellPrice, function(res2){
												if(!res.error){
